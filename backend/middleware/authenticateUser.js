...
export const isAuthenticated = async (req, res, next) => {
  try {
    ...
    if (!jwtToken && !localToken) {
      return res
        .status(401)
        .json({ message: "Unauthorized access, You need to login first." });
    }

    const decoded = jwt.verify(
      jwtToken ? jwtToken : localToken,
...
export const isAdmin = async (req, res, next) => {
  try {
    ...
    if (find_group?.createdBy?.toString() != loggedUserId.toString()) {
      return res
        .status(403)
        .json({ message: "Only Admin Of Group Have Permission" });
    }
