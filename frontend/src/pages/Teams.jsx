...
const initialTeams = [
  {
    ...
    tasks: [
      { id: 't1', title: 'Homepage redesign', assignee: 'Alice Johnson', status: 'In Progress', priority: 'High' },
      { id: 't2', title: 'Mobile mockups',    assignee: 'Vishal Patidar', status: 'Pending',     priority: 'Medium' },
      { id: 't3', title: 'Icon set v2',       assignee: 'Bob Smith',      status: 'Completed',  priority: 'Low' }
    ],
    activeTasks: 12,
    completedTasks: 45,
    progress: 78,
  },
  {
    ...
    tasks: [
      { id: 't4', title: 'API integration', assignee: 'John Doe',        status: 'Completed',  priority: 'High' },
      { id: 't5', title: 'Bug fix #234',    assignee: 'Jane Smith',      status: 'In Progress',priority: 'Critical' },
      { id: 't6', title: 'Auth refactor',   assignee: 'Vishal Patidar',  status: 'In Progress',priority: 'High' },
    ],
    activeTasks: 23,
    completedTasks: 127,
    progress: 65,
  },
  {
    ...
    tasks: [
      { id: 't7', title: 'Q4 Campaign',          assignee: 'Sarah Williams', status: 'Completed', priority: 'Medium' },
      { id: 't8', title: 'Social media plan',    assignee: 'Mike Johnson',   status: 'In Progress', priority: 'Low' },
      { id: 't9', title: 'Newsletter template',  assignee: 'Vishal Patidar', status: 'Pending',   priority: 'Low' },
    ],
    activeTasks: 8,
    completedTasks: 32,
    progress: 82,
  },
  {
    ...
    tasks: [
      { id: 't10', title: 'Feature roadmap', assignee: 'Tom Brown',       status: 'In Progress', priority: 'High' },
      { id: 't11', title: 'User research',   assignee: 'Emily Davis',     status: 'Completed',   priority: 'Medium' },
      { id: 't12', title: 'Spec v1.2',       assignee: 'Vishal Patidar',  status: 'Pending',     priority: 'High' },
    ],
    activeTasks: 15,
    completedTasks: 89,
    progress: 71,
...
const Teams = () => {
  ...
  const removeMember = (memberName) => {
    ...
    setTeams(prev => prev.map(t => {
      if (t.id !== selectedTeam.id) return t;
      return { ...t, members: t.members.filter(m => m.name !== memberName) };
    }));
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="mt-1 text-gray-600 dark:text-gray-400">
              {selectedTeam ? 'Team overview, members, and tasks' : 'Manage your teams and track their progress'}
            </p>
          </div>
        </div>
              ...
              <div onClick={() => handleSelect(team.id)} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                    <div className={`h-full ${team.color} transition-all duration-500`} style={{ width: `${team.progress}%` }} />
                  </div>
                </div>
              </div>
            </Card>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700">
                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-700 dark:text-gray-300">Title</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-700 dark:text-gray-300">Assignee</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-700 dark:text-gray-300">Status</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-700 dark:text-gray-300">Priority</th>
                  </tr>
                </thead>
                <tbody>
      ...
      {showCreate && (
        <InlineModal title="Create Group" onClose={() => setShowCreate(false)}>
          <CreateGroupForm
            onCreate={(name) => { createGroup(name); setShowCreate(false); }}
            onCancel={() => setShowCreate(false)}
          />
        </InlineModal>
      )}

      {showAdd && selectedTeam && (
        <InlineModal title={`Add Member to ${selectedTeam.name}`} onClose={() => setShowAdd(false)}>
          <AddMemberForm
            onAdd={(name, role) => { addMember(name, role); setShowAdd(false); }}
            onCancel={() => setShowAdd(false)}
          />
        </InlineModal>
      )}

      {showRemove && selectedTeam && (
        <InlineModal title={`Remove Member from ${selectedTeam.name}`} onClose={() => setShowRemove(false)}>
          <RemoveMemberForm
            members={selectedTeam.members}
            onRemove={(name) => { removeMember(name); setShowRemove(false); }}
      ...
      {showCreateTask && selectedTeam && (
        <InlineModal title={`Create Task in ${selectedTeam.name}`} onClose={() => setShowCreateTask(false)}>
          <CreateTaskForm
            members={selectedTeam.members}
            onCreate={(payload) => {
...
const AddMemberForm = ({ onAdd, onCancel }) => {
  ...
  return (
    <form
      onSubmit={(e) => { e.preventDefault(); if (name.trim()) onAdd(name.trim(), role.trim()); }}
      className="space-y-4"
    >
      <div>
        <label className="block text-sm mb-1 text-gray-700 dark:text-gray-300">Member Name</label>
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="w-full rounded-md bg-gray-50 dark:bg-gray-700/50 px-3 py-2 outline-none"
...
const RemoveMemberForm = ({ members, onRemove, onCancel }) => {
  ...
  return (
    <form
      onSubmit={(e) => { e.preventDefault(); if (name) onRemove(name); }}
      className="space-y-4"
    >
      <div>
        <label className="block text-sm mb-1 text-gray-700 dark:text-gray-300">Select Member</label>
        <select
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="w-full rounded-md bg-gray-50 dark:bg-gray-700/50 px-3 py-2 outline-none"
...
const CreateTaskForm = ({ members, onCreate, onCancel }) => {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm mb-1 text-gray-700 dark:text-gray-300">Assign to</label>
        <select
          value={assigneeId}
